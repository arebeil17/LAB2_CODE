...
void storePassword(char set[], char (*db)[5], int row);
...
int main(void)
{
    ...
    {        
        switch(state){
            ...
            case ENTER:
                if(char_index == 0) {
                    moveCursorLCD(1,1); printStringLCD("Enter");
                    moveCursorLCD(1,2); 
                }
                turnOnLED(1);
                scanAllRows();
                ...
                if((current_char == '*') && (char_index == 0)) star_count = 1;
                else if((star_count == 1)&&(current_char == '*')&&(char_index == 1)){
                    star_count = 0;
                    char_index = 0;
                    current_char = 0; resetToggle(0); kp_cnt = 0;
                    state = SET_MODE; clearLCD(); break;
                ...
                if(char_index == 4){ 
                    char_index = 0; 
                    for(k = 0; k < 4; k++){
                        if(passwordCheck(new, database, k )){//add arguments
                            mode = good;
                            state = STATUS; break;
                        }else if(k == 3){
                            mode = bad;
